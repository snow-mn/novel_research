...
def get_all_novel_info():
    ...
    # 出力パラメータの設定
    # 総合ポイントの下限と文字数の上限を設定
    payload = {'out': 'json', 'gzip': 5, 'of': 'n', 'lim': 1, "general_all_no": 2-10000 ,"min_globalpoint": 1000, "length": -500000}
    # レスポンスをcontent形式で取得
    ...
    for i in tqdm(range(all_queue_cnt)):
        payload = {'out': 'json', 'gzip': 5, 'opt': 'weekly', 'lim': 500, 'lastup': "1073779200-" + str(lastup), "general_all_no": 2-10000, "min_globalpoint": 1000, "length": -500000}
...
def count_word_ranking(file_path):
    ...
    # ファイルパス
    keywordcount_xlsx_path = data_dir + "keyword_count_%s.xlsx" % now_day
    # データフレーム化
    df = pd.DataFrame(sorted_dict[0:200], columns=["keyword", "count"])
    print(df)
    # エクセルファイルを作成・保存
    df.to_excel(keywordcount_xlsx_path, sheet_name="keyword_count")
    print(keywordcount_xlsx_path + "の保存が完了しました")
