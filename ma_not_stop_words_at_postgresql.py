...
# 格納するデータベース名
database_name = "ma_data_not_stop_words"
...
def morphological_analysis(connection, ncode):
    ...
    for line in tqdm(lines):
        ...
        for tok in doc:
            # ストップワードを除外する
            if not tok.is_stop and tok.pos_ not in ["PUNCT", "SYM"]:
                # 要素の追加
                ncode_list += [ncode]
                line_index += [line_num]
                token_index += [tok.i]
    ...
    ma_df = pd.DataFrame(
        data={"ncode": ncode, "line_index": line_index, "token_index": token_index, "token_text": token_text,
              ...
              "token_dep": token_dep, "token_norm": token_norm,
              "token_ent_iob": token_ent_iob, "token_ent_type": token_ent_type,
